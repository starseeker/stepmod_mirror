...
<!--
$Id: ap_list.xsl,v 1.2 2003/05/30 05:57:07 mikeward Exp $
  Author:  Mike Ward, Eurostep Limited
  Owner:   Developed by Eurostep Limited
  Purpose: A set of imported templates to set up a list of business_object_models
  The importing file will define the business_object_models template which
  displays the list of business_object_models
-->
...
<xsl:template match="repository_index">
  <HTML>
    <head>
      ...
      <title>
        <xsl:value-of select="business_object_models" />
      </title>
    ...
    <body>
      <xsl:apply-templates select="business_object_models"/>
    </body>
  </HTML>
</xsl:template>
...
<xsl:template match="business_object_model">
  ...
  <xsl:variable name="href" 
    select="concat('../data/business_object_models/',@name,'/home',$FILE_EXT)"/>
  ...
  <div id="{$Menu}" style="display:none">
    ...
    <xsl:variable name="arm" 
      select="concat('../data/business_object_models/',@name,'/sys/4_info_reqs',$FILE_EXT)"/>

    <xsl:variable name="armg" 
      select="concat('../data/business_object_models/',@name,'/armexpg1',$FILE_EXT)"/>
    <p class="menuitem">
    ...
    <xsl:variable name="mim" 
      select="concat('../data/business_object_models/',@name,'/sys/5_main',$FILE_EXT)"/>
    <xsl:variable name="mimg" 
      select="concat('../data/business_object_models/',@name,'/mimexpg1',$FILE_EXT)"/>
    <p class="menuitem">
