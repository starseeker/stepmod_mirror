...
<xsl:template match="/" >
  ...
  <HTML>
	...
	<p>
	Shows a list of resource schemas followed by a table that is ordered by entity 
	and shows those Arm elements (and their respective modules) that use the resource entity in the mapping.
	...
	<xsl:when test="function-available('msxsl:node-set')">
              ...
              <xsl:apply-templates select="$rep_index//resources/resource" mode="map">
...
<xsl:template match="mapping" mode="map">
	...
	<xsl:choose>
		<xsl:when test="@attribute" >	
			<a href="../data/modules/{$mod_name}/sys/5_mapping{$FILE_EXT}#{@entity}.{@attribute}">
			<xsl:value-of select="@entity" />.<xsl:value-of select="@attribute" />
			</a>
		</xsl:when>
		<xsl:otherwise>
			<a href="../data/modules/{$mod_name}/sys/5_mapping{$FILE_EXT}#{@entity}">
				<xsl:value-of select="@entity" />
			</a>
		</xsl:otherwise>
	</xsl:choose>
	[<a href="../data/modules/{$mod_name}/sys/5_mapping{$FILE_EXT}#mappings">
	 <xsl:value-of select="../@name" />
	 </a>]
	<br/>
