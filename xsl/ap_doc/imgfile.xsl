    ...
    <body>
      ...
      <xsl:choose>
        <xsl:when test="@application_protocol">
          ...
          <xsl:apply-templates 
            select="document($application_protocol_file)/application_protocol"
          ...
          <xsl:choose>
            <xsl:when test="./@file">
              <xsl:apply-templates 
                select="document($application_protocol_file)/application_protocol/*/express-g/imgfile"
                mode="nav_arrows">
        ...
        <xsl:otherwise>
          <xsl:call-template name="error_message">
            <xsl:with-param name="message">
              <xsl:value-of 
                select="'Error IM1: Error in image file - application_protocol not specified'"/>
            </xsl:with-param>
          </xsl:call-template>
          <xsl:apply-templates select="img"/>
          <div align="center"><h3><xsl:value-of select="@title"/></h3></div>
    ...
    <!-- Page navigation:&#160; -->
      ...
      <xsl:variable name="home">
        <xsl:choose>
          ...
          <xsl:when test="name(../..)='mim'">
            <xsl:call-template name="set_file_ext">
              <xsl:with-param name="filename" select="'./sys/d_aim_expg.xml'"/>
            </xsl:call-template>
          </xsl:when>
        </xsl:choose>
      </xsl:variable>
