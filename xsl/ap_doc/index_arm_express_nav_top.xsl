...
-->
<!--
$Id: index_arm_express_nav_top.xsl,v 1.8 2004/10/21 14:48:23 nigelshaw Exp $
...
<xsl:template name="arm_express_index" >
	...
	<xsl:param name="called-schemas" />
		...
		<xsl:if test="string-length($const-names) > 1" >
			<A HREF="index_arm_express_nav_inner{$FILE_EXT}#constants" TARGET="toc_inner" ><B>Constants:</B></A>
			<xsl:text> </xsl:text>
			<xsl:call-template name="alph-index">
				<xsl:with-param name="names" select="$const-names" />
				<xsl:with-param name="file" select="concat('index_arm_express_nav_inner',$FILE_EXT)" />
		...
		<xsl:if test="string-length($type-names) > 1" >
			<A HREF="index_arm_express_nav_inner{$FILE_EXT}#types" TARGET="toc_inner" ><B>Types:</B></A>
			<xsl:text> </xsl:text>
			<xsl:call-template name="alph-index">
		...
		<xsl:if test="string-length($ent-names) > 1" >
			<A HREF="index_arm_express_nav_inner{$FILE_EXT}#entities" TARGET="toc_inner" ><B>Entities:</B></A>
			<xsl:text> </xsl:text>
			<xsl:call-template name="alph-index">
		...
		<xsl:if test="string-length($subc-names) > 1" >
			<A HREF="index_arm_express_nav_inner{$FILE_EXT}#subc" TARGET="toc_inner" ><B>Subtype Constraints:</B></A>
			<xsl:text> </xsl:text>
			<xsl:call-template name="alph-index">
		...
		<xsl:if test="string-length($rule-names) > 1" >
			<A HREF="index_arm_express_nav_inner{$FILE_EXT}#rules" TARGET="toc_inner" ><B>Rules:</B></A>
			<xsl:text> </xsl:text>
			<xsl:call-template name="alph-index">
		...
		<xsl:if test="string-length($fun-names) > 1" >
			<A HREF="index_arm_express_nav_inner{$FILE_EXT}#functions" TARGET="toc_inner" ><B>Functions:</B></A>
			<xsl:text> </xsl:text>
			<xsl:call-template name="alph-index">
		...
		<xsl:if test="string-length($proc-names) > 1" >
			<A HREF="index_arm_express_nav_inner{$FILE_EXT}#procedures" TARGET="toc_in	ner" ><B>Procedures:</B></A>
			<xsl:text> </xsl:text>
			<xsl:call-template name="alph-index">
