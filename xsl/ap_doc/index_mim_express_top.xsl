...
-->
<!--
$Id: index_mim_express_top.xsl,v 1.2 2003/06/06 16:15:17 nigelshaw Exp $
...
-->
  ...
  <body>
	...
	<xsl:variable name="mim_constant_names" >
		...
		<xsl:if test="string-length($mim_constant_names) > 1" >
			<A HREF="index_mim_express_inner{$FILE_EXT}#constants" TARGET="toc_inner" ><B>Constants:</B></A>
			<xsl:text> </xsl:text>
			<xsl:call-template name="alph-index">
				<xsl:with-param name="names" select="$mim_constant_names" />
				<xsl:with-param name="file" select="concat('index_mim_express_inner',$FILE_EXT)" />
	...
	<xsl:variable name="mim_type_names" >
		...
		<xsl:if test="string-length($mim_type_names) > 1" >
			<A HREF="index_mim_express_inner{$FILE_EXT}#types" TARGET="toc_inner" ><B>Types:</B></A>
			<xsl:text> </xsl:text>
			<xsl:call-template name="alph-index">
				<xsl:with-param name="names" select="$mim_type_names" />
				<xsl:with-param name="file" select="concat('index_mim_express_inner',$FILE_EXT)" />
	...
	<xsl:variable name="mim_entity_names" >
		...
		<xsl:if test="string-length($mim_entity_names) > 1" >
			<A HREF="index_mim_express_inner{$FILE_EXT}#entities" TARGET="toc_inner" ><B>Entities:</B></A>
			<xsl:text> </xsl:text>
			<xsl:call-template name="alph-index">
				<xsl:with-param name="names" select="$mim_entity_names" />
				<xsl:with-param name="file" select="concat('index_mim_express_inner',$FILE_EXT)" />
	...
	<xsl:variable name="mim_rule_names" >
		...
		<xsl:if test="string-length($mim_rule_names) > 1" >
			<A HREF="index_mim_express_inner{$FILE_EXT}#rules" TARGET="toc_inner" ><B>Rules:</B></A>
			<xsl:text> </xsl:text>
			<xsl:call-template name="alph-index">
				<xsl:with-param name="names" select="$mim_rule_names" />
				<xsl:with-param name="file" select="concat('index_mim_express_inner',$FILE_EXT)" />
	...
	<xsl:variable name="mim_function_names" >
		...
		<xsl:if test="string-length($mim_function_names) > 1" >
			<A HREF="index_mim_express_inner{$FILE_EXT}#functions" TARGET="toc_inner" ><B>Functions:</B></A>
			<xsl:text> </xsl:text>
			<xsl:call-template name="alph-index">
				<xsl:with-param name="names" select="$mim_function_names" />
				<xsl:with-param name="file" select="concat('index_mim_express_inner',$FILE_EXT)" />
	...
	<xsl:variable name="mim_procedure_names" >
		...
		<xsl:if test="string-length($mim_procedure_names) > 1" >
			<A HREF="index_mim_express_inner{$FILE_EXT}#procedures" TARGET="toc_inner" ><B>Procedures:</B></A>
			<xsl:text> </xsl:text>
			<xsl:call-template name="alph-index">
				<xsl:with-param name="names" select="$mim_procedure_names" />
				<xsl:with-param name="file" select="concat('index_mim_express_inner',$FILE_EXT)" />
