...
-->
<!--
$Id: index_mim_modules_inner.xsl,v 1.13 2004/12/29 14:29:24 robbod Exp $
...
<xsl:template name="modules_index" >
	...
	<xsl:param name="called-schemas" />
		...
		<xsl:call-template name="alph-list">
			<xsl:with-param name="items" select="$called-schemas//schema[contains(@name,'_mim')]" />
...
<xsl:template match="schema" mode="module-index" >
		...
		<xsl:variable name="mod-name" select="translate(substring-before(@name,'_mim'),$UPPER,$LOWER)" />
