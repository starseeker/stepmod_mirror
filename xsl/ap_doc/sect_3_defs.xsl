...
<!--
$Id: sect_3_defs.xsl,v 1.9 2003/05/27 14:24:48 robbod Exp $
        ...
        <xsl:when test="contains($first,'normref:')">
          ...
          <xsl:if test="$module_number!=$part_no">
            ...
            <ul>
              ...
              <xsl:apply-templates 
                select="document('../../data/basic/ap_doc/normrefs_default.xml')/normrefs/normref.inc[@normref=$ref]/term.ref" mode="normref"/>
              <!-- check to see if any terms from the same normref are 
                   identified in module -->
              <xsl:apply-templates 
                select="/application_protocol/normrefs/normref.inc[@normref=$ref]/term.ref"
                mode="normref"/>
              <xsl:apply-templates 
...
-->
<xsl:template match="application_protocol" mode="normrefs_terms_list">
...
<xsl:template match="abbreviations" mode="output">
  <!-- output any abbreviations defined in the application_protocol -->
