...
<!--
$Id: sect_a_exp_aim_lf.xsl,v 1.3 2002-07-17 07:50:08 mwd Exp $
  Author:  Mike Ward, Eurostep Limited
  Owner:   Developed by Eurostep and supplied to NIST under contract.
  Purpose: Display the AIM long form express 
...
<!-- debug <xsl:message>  global_xref_list defined in sect_e_exp_arm.xsl </xsl:message> -->
		<xsl:choose>
			<xsl:when test="/application_protocol_clause">
				...
				<xsl:variable name="express_xml" select="concat($application_protocol_dir,'/aim_lf.xml')"/>
	...
	<xsl:template match="application_protocol">
		...
		<xsl:call-template name="clause_header">
			...
			<xsl:with-param name="aname" select="'annexa-aim_lf-express'"/>
		</xsl:call-template>
		...
		<xsl:variable name="aim_lf_xml" select="concat($application_protocol_dir,'/aim_lf.xml')"/>
		
		<!-- get the express and display using the annex_express.xsl stylesheet that has been imported. -->
