...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" version="1.0">
	...
	<xsl:variable name="global_xref_list">
		<xsl:choose>
			<xsl:when test="/application_protocol_clause">
				...
				<xsl:variable name="express_xml" select="concat($application_protocol_dir,'/mim_lf.xml')"/>
	...
	<xsl:template match="application_protocol">
		<xsl:call-template name="clause_header">
			<xsl:with-param name="heading" select="'Annex G - AIM Long form EXPRESS'"/>
			<xsl:with-param name="aname" select="'annexg-aim_lf-express'"/>
		</xsl:call-template>
		<xsl:variable name="ap_module_dir">
			<xsl:call-template name="ap_module_directory">
