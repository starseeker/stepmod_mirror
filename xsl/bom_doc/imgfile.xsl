...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" version="1.0">
 <!-- <xsl:import href="../imgfile.xsl"/>-->
  ...
  <xsl:template match="imgfile.content">
    ...
    <xsl:variable name="model_dir" select="@business_object_model"/>
      
    <xsl:variable name="model_file" select="concat('../../data/business_object_models/', $model_dir,'/business_object_model.xml')"/>

    <xsl:variable name="bom_xml" select="document($model_file)"/>
    ...
    <HTML>
      <HEAD>
        <xsl:if test="@business_object_model">
          <xsl:apply-templates select="$bom_xml/business_object_model" mode="meta_data"/>
        </xsl:if>        
        <TITLE>
          <xsl:apply-templates 
            select="$bom_xml/business_object_model" mode="title"/>
        </TITLE>
      </HEAD>
      <body>
        <xsl:variable name="self" select="."/>
        <xsl:choose>
          <xsl:when test="@business_object_model">
           <!-- <xsl:apply-templates select="$bom_xml/application_protocol" mode="TOCmultiplePage">
