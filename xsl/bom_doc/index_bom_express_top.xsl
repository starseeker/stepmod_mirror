...
<!--      <xsl:apply-templates select="$ap_node/application_protocol" mode="meta_data"/>-->
      <title>
	      <xsl:value-of select="concat('BOM Index for ',$ap_node/application_protocol/@title)"/>
      </title>
  ...
  <small> 
	...
	<xsl:variable name="schema-name"
	    select="$bom_xml_node//schema/@name"/>
			...
			<xsl:call-template name="bom_express_index" >
...
<xsl:template name="bom_express_index" >
	<xsl:param name="called-schemas" />
		...
		<xsl:if test="string-length($const-names) > 1" >
			<A HREF="index_bom_express_inner{$FILE_EXT}#constants" TARGET="toc_inner" ><B>Constants:</B></A>
			<xsl:text> </xsl:text>
			<xsl:call-template name="alph-index">
				<xsl:with-param name="names" select="$const-names" />
				<xsl:with-param name="file" select="concat('index_bom_express_inner',$FILE_EXT)" />
		...
		<xsl:if test="string-length($type-names) > 1" >
			<A HREF="index_bom_express_inner{$FILE_EXT}#types" TARGET="toc_inner" ><B>Types:</B></A>
			<xsl:text> </xsl:text>
			<xsl:call-template name="alph-index">
		...
		<xsl:if test="string-length($ent-names) > 1" >
			<A HREF="index_bom_express_inner{$FILE_EXT}#entities" TARGET="toc_inner" ><B>Entities:</B></A>
			<xsl:text> </xsl:text>
			<xsl:call-template name="alph-index">
		...
		<xsl:if test="string-length($subc-names) > 1" >
			<A HREF="index_bom_express_inner{$FILE_EXT}#subc" TARGET="toc_inner" ><B>Subtype Constraints:</B></A>
			<xsl:text> </xsl:text>
			<xsl:call-template name="alph-index">
		...
		<xsl:if test="string-length($rule-names) > 1" >
			<A HREF="index_bom_express_inner{$FILE_EXT}#rules" TARGET="toc_inner" ><B>Rules:</B></A>
			<xsl:text> </xsl:text>
			<xsl:call-template name="alph-index">
		...
		<xsl:if test="string-length($fun-names) > 1" >
			<A HREF="index_bom_express_inner{$FILE_EXT}#functions" TARGET="toc_inner" ><B>Functions:</B></A>
			<xsl:text> </xsl:text>
			<xsl:call-template name="alph-index">
		...
		<xsl:if test="string-length($proc-names) > 1" >
			<A HREF="index_bom_express_inner{$FILE_EXT}#procedures" TARGET="toc_inner" ><B>Procedures:</B></A>
			<xsl:text> </xsl:text>
			<xsl:call-template name="alph-index">
