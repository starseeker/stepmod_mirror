  ...
  <xsl:template match="business_object_model_clause">
    ...
    <html>
      <head>
        ...
        <title>BOM Mappings</title>
      </head>
      <!--    <frameset rows="25,75"> -->
      <frameset rows="25,125"> 
      <frame name="toc_top" 
        src="../../../../data/business_object_models/{$selected_bom}/sys/index_bom_mappings_top{$FILE_EXT}"
        frameborder="0"
        marginwidth="2" marginheight="0" />
        <frame name="toc_inner" 
          src="../../../../data/business_object_models/{$selected_bom}/sys/index_bom_mappings_inner{$FILE_EXT}" 
          frameborder="0"
