...
<!--
  ...
  Purpose: 
  	1) To display a complete index of mappings from the BOM level model of the AP
	2) To check that the index mappings exist. 
	NOTE: Mappings are indexed by examining the BOM EXPRESS and deriving the list of mappings,
...
<xsl:variable name="selected_bom"
	select="/business_object_model_clause/@directory"/>
  ...
  <xsl:variable name="iframe-width"
    ...
    <xsl:variable name="bom_xml_node"
	    select="document($bom_xml_file)"/>


    <xsl:variable name="called-modules" />
...
<!--    <xsl:apply-templates select="$ap_node" mode="meta_data"/> -->
      <title>
        <xsl:value-of select="concat('Mappings Index for ',$selected_ap,' BOM')"/>
      </title>

    </head>
  <body>
    <xsl:apply-templates select="/" mode="bom_mappings"/>	

  </body>
</HTML>
</xsl:template>

<xsl:template match="/" mode="bom_mappings" >

	<xsl:variable name="schema-name"
	    select="$bom_xml_node//schema/@name"/>

		<xsl:call-template name="alph-list">
			<xsl:with-param name="items" select="$bom_xml_node//entity" />
...
<xsl:template match="entity" mode="module-index" >
		...
		<xsl:variable name="bom-dir" select="concat('../../../business_object_models/',$mod-name)" />
		<xsl:variable name="ent-name" select="@name" />
		<xsl:variable name="lc-ent" select="translate(@name,$UPPER,$LOWER)" />

		<br/>
		<A HREF="{$bom-dir}/sys/5_main{$FILE_EXT}#beentity{$lc-ent}" target="info">
                  <xsl:value-of select="@name"/>
		</A>
		<br/>
                <!-- check that mapping exists -->
			<xsl:if test="not($bom-node//be[@entity=$ent-name])" >
			...
			<xsl:call-template name="error_message">
			  ...
			  <xsl:with-param name="warning_gif" select="'../../../../images/warning.gif'"/>
		          <xsl:with-param 
		            name="message" 
		            select="concat('Error BOMmapindex1: Unable to locate mapping for entity ',$ent-name)"/>
			</xsl:call-template>    	
...
<!--			<xsl:value-of select="concat(' ',../@name,'.',@name)"/> -->
                    <small>
			<A 
			HREF="{$bom-dir}/sys/5_main{$FILE_EXT}#beentity{$lc-ent}baattribute{@name}assertion_to{$lc-typename}" 
			target="info">
			<xsl:value-of select="concat(' ',../@name,'.',@name)"/>
			</A>
                      </small>
...
-->
			...
			<xsl:when 
			  test="$called-modules//module[@name=$mod-name]//ae[@entity=$ent-name]//aa[@attribute=$attr-name]">
				...
				<A 
			HREF="{$bom-dir}/sys/5_main{$FILE_EXT}#beentity{$lc-ent}baattribute{@name}" 
				target="info">
                                  <small><xsl:value-of select="concat(' ',../@name,'.',@name)"/></small>
				</A>
				<br/>
...
<!--			Mapping NOT found -->
			...
			<xsl:when 
			  test="$called-modules//module[@name=$mod-name]//ae[@entity=$ent-name]//aa
				...
				<A 
                                  HREF="{$bom-dir}/sys/5_main{$FILE_EXT}#beentity{$lc-ent}baattributeself{$redeclared-from}.{@name}assertion_to{$lc-typename}" 
				target="info">
                                  <small><xsl:value-of select="concat(' ',../@name,'.',@name)"/></small>
				</A>
...
<!--				Mapping NOT found q -->
		...
		<xsl:otherwise>
			<A HREF="{$bom-dir}/sys/5_main{$FILE_EXT}#beentity{$lc-ent}baattribute{@name}" target="info">
                          <small><xsl:value-of select="concat(' ',../@name,'.',@name)"/></small>
			</A>

			<!-- check that mapping exists -->
			<xsl:if test="not($bom_xml_node//be[@entity=$ent-name]//ba[@attribute=$attr-name])">
<!--				Mapping NOT found p -->
				<xsl:call-template name="error_message">
				  <xsl:with-param name="inline" select="'yes'"/>
				  <xsl:with-param name="warning_gif" select="'../../../../images/warning.gif'"/>
			          <xsl:with-param 
			            name="message" 
			            select="concat('Error BOMmapindex2: Unable to locate mapping for attribute ',
				    $ent-name,'.',$attr-name)"/>
				</xsl:call-template>    	
			</xsl:if>
