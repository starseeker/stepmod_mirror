    ...
    <xsl:choose>
      <xsl:when test="/module_clause">
        <xsl:variable name="model_dir">
          <xsl:call-template name="model_directory">
            <xsl:with-param name="module" select="/module_clause/@directory"/>
          </xsl:call-template>
        </xsl:variable>
        
        <xsl:variable name="express_xml" select="concat($model_dir,'/bom.xml')"/>
      ...
      <xsl:when test="/module">
        ...
        <xsl:variable name="express_xml" select="concat($model_dir,'/arm.xml')"/>
        <xsl:call-template name="build_xref_list">
          <xsl:with-param name="express" select="document($express_xml)/express"/>
        </xsl:call-template>        
      </xsl:when>
    ...
    <p class="note">
      <small>
        ...
        <xsl:variable name="module_file"
          select="concat($model_dir,'/module.xml')"/>
  ...
  <xsl:variable name="clause_intro">
    <xsl:choose>
          <xsl:when test="count(../type)>1">
            This subclause specifies the types
            for this business object model. The BO model types and 
            definitions are specified below.         
          </xsl:when>
          <xsl:otherwise>
            This subclause specifies the type
            for this business object model. The BO model type and 
            its definition are specified below.   
          </xsl:otherwise>
        </xsl:choose>
  </xsl:variable>
...
<xsl:template name="express_clause_present">
  ...
  <xsl:variable name="xml_file">
    <xsl:choose>
      <xsl:when test="contains($schema_name,'_arm')">
        <xsl:value-of select="concat($model_dir,'/arm.xml')"/>
      </xsl:when>
      <xsl:when test="contains($schema_name,'_mim')">
        <xsl:value-of select="concat($model_dir,'/mim.xml')"/>
      </xsl:when>
      <xsl:otherwise>
...
<xsl:template name="express_clause_number">
  ...
  <xsl:variable name="model_dir">
    <xsl:call-template name="model_directory">
      <xsl:with-param name="model" select="$schema_name"/>
    </xsl:call-template>
  </xsl:variable>

  <xsl:variable name="xml_file">
    <xsl:value-of select="concat($model_dir,'/bom.xml')"/>   
  </xsl:variable>
...
<xsl:template match="described.item">
  ...
  <xsl:variable name="model_dir">
    <xsl:call-template name="model_directory">
      <xsl:with-param name="model" select="../@schema"/>
    </xsl:call-template>
  </xsl:variable>
  <xsl:variable name="module_no"
    select="document(concat($model_dir,'/module.xml'))/module/@part"/>
...
-->
  ...
  <xsl:template name="model_directory">
  ...
  <xsl:template name="model_name">
    ...
    <xsl:variable name="model_lcase"
      select="translate(
      normalize-space(translate($model,$UPPER, $LOWER)),
      '&#x20;','_')"/>
