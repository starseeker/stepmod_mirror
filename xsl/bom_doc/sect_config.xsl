...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:xsd="http://www.w3.org/2001/XMLSchema" version="1.0">
  ...
  <xsl:template match="business_object_model_clause">
    ...
    <xsl:variable name="business_object_model_config_file" select="concat('../../data/business_object_models/',@directory,'/p28_config.xml')"/>

    <xsl:variable name="business_object_model_config" select="document($business_object_model_config_file)"/>
    ...
    <html>
      ...
      <body>
        <xsl:apply-templates select="$business_object_model_config" mode="create_html"/>
      </body>
    </html>
  ...
  <xsl:template match="/" mode="create_html">
    <h1>XML Schema Configuration File listing for <xsl:value-of select="$bom_name"/></h1>
   <code>
     <xmp>
