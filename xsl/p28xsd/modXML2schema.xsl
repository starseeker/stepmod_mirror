...
<!--
	$Id: modXML2schema.xsl,v 1.1 2007/06/15 16:55:34 robbod Exp $
	Author:  Mike Ward, Eurostep Limited
	Owner:   Developed by Eurostep.
	Purpose:     generation of p28 XSD from expressXML


     The program uses namespace.xml and exp_namespace.xml to set the
     namespaces of the P28 schema

     java -jar c:/apps/saxon6-5-5/saxon.jar -o ../../../data/dex/aviation_maintenance/dex_lf.xsd ../../../data/dex/aviation_maintenance/dex_lf.xml modXML2schema.xsl namespaceXml="file:///D:/users/rbn/sforge/plcs/dexlib/apps/dexlib/xsl/namespace1.xml"  expNamespaceXml="file:///D:/users/rbn/sforge/plcs/dexlib/apps/dexlib/xsl/exp_namespace1.xml"
...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:xs="http://www.w3.org/2001/XMLSchema" xmlns:exp="urn:iso:std:iso:10303:28:ed-2:2005:schema:common">
	...
	<!--MWD-->
        ...
        <xsl:param name="expNamespaceXml"/>-->
	<!--MWD-->
	<xsl:variable name="stepmod_namespace_filename" select="'stepmod_namespace.xml'"/>
          
        

	<!--MWD-->
	<xsl:variable name="exp_namespace_filename" select="'exp_namespace.xml'"/>
          

	<xsl:variable name="stepmod_namespace_file" select="document(concat($directory_path, '/', $stepmod_namespace_filename))"/>
	...
	<xsl:template match="schema">
		...
		<xsl:element name="xs:schema">
			<!--MWD-->
			<xsl:copy-of select="document(concat('../../dtd/part28/', $exp_namespace_filename))/*/namespace::exp"/>
			<xsl:copy-of select="document(concat($directory_path, '/', $stepmod_namespace_filename))/*/namespace::*"/>
	...
	<xsl:template match="explicit" mode="keys">
		...
		<xsl:variable name="abstract">
			<xsl:if test="//entity[@name=$raw_target_name]/@abstract.supertype"><xsl:value-of select="'YES'"/></xsl:if>
		</xsl:variable>
		<xsl:choose>
	...
	<xsl:template name="collect_attributes_from_supertypes_but_weed_those_redeclared_lower_down">
		...
		<xsl:variable name="next_supertype_name_list">
			<xsl:for-each select="//entity[contains($supertype_name_list_with_spaces, concat(' ', ./@name, ' '))]">
				<xsl:value-of select="concat(./@supertypes, ' ')"/>
			</xsl:for-each>
		</xsl:variable>
		<xsl:variable name="new_list_of_redeclared_attributes">
			<xsl:for-each select="//entity[contains($raw_entity_param_with_spaces, concat(' ', @name, ' '))]/explicit/redeclaration">
				<xsl:choose>
		...
		<xsl:for-each select="//entity[contains($supertype_name_list_with_spaces, concat(' ', @name, ' '))]/explicit">
			<xsl:variable name="raw_attribute_name" select="./@name"/>
