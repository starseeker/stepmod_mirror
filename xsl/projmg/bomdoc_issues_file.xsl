...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" version="1.0">
  <xsl:import href="../bom_doc/business_object_model.xsl"/>
  <xsl:import href="../bom_doc/business_object_model_toc.xsl"/>
  ...
  <xsl:template match="issues">
    <xsl:variable name="bomdoc_name" select="@bom_doc"/>
    <xsl:variable name="open_issues"
      select="count(//issue[@status!='closed'])"/>
    <xsl:if test="$open_issues>0">
      <xsl:call-template name="error_message">
        <xsl:with-param name="message">
          <xsl:value-of select="concat('Warning Issues1: ','BOM doc ', $bomdoc_name, ' has ',$open_issues , ' open issues' )"/>
        </xsl:with-param>
    ...
    <xsl:variable name="bomdoc_file"
      select="concat('../../data/business_object_models/',@bom_doc,'/business_object_model.xml')"/>
    <xsl:variable name="in_repo"
      select="document('../../repository_index.xml')/repository_index/business_object_models/business_object_model[@name=$bomdoc_name]"/>

    <html>
      <title>
        <xsl:value-of select="concat(/issues/@bom_doc,' - issues')"/>
      </title>
      ...
      <body>
        ...
        <xsl:choose>
          ...
          <xsl:when test="$in_repo">
            <xsl:variable name="bomdoc_node"
              select="document($bomdoc_file)/business_object_model[@name=$bomdoc_name]"/>
            <xsl:apply-templates select="$bomdoc_node" mode="TOCmultiplePage"/> 
          </xsl:when>
          <xsl:otherwise>
            <p>
              <xsl:call-template name="error_message">
                ...
                <xsl:with-param name="message">
                  <xsl:value-of select="concat('Error B1: Business_object_model ',
                                        $bomdoc_name,
