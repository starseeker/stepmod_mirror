...
>
  ...
  <HTML>
    <HEAD>
      ...
      <TITLE>Integrated generic resources  repository</TITLE>
    </HEAD>
    <BODY>
      <table width="600">
        <tr>
...
<xsl:template match="resource_docs">
  <h2>
    <a name="alphatop">
      Alphabetical list of Integrated generic resources
    </a>
  </h2>
  <xsl:variable name="resource_doc_mid_point" select="(count(./resource_doc)+1) div 2"/>
  <blockquote>
    <table width="90%" cellspacing="0" cellpadding="4">
      <tr>
        <td valign="top">
          <xsl:apply-templates select="./resource_doc" mode="col1">
            <xsl:with-param name="mid_point" select="$resource_doc_mid_point"/>
            <xsl:sort select="@name"/>
          </xsl:apply-templates>
        </td>
        <td valign="top">
          <xsl:apply-templates select="./resource_doc" mode="col2">
            <xsl:with-param name="mid_point" select="$resource_doc_mid_point"/>
            <xsl:sort select="@name"/>
          </xsl:apply-templates>
        </td>
